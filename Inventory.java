package com.polban.jtk.inventory;

public class Inventory {
    Barang[] barangs;

    void initBarang() {
        barangs = new Barang[2];
        barangs[0] = new Barang("001", "Baju", 10);
        barangs[1] = new Barang("002", "Celana", 20);
    }

    void showBarang() {
        System.out.println(barangs[0].getNamaBarang() + " (" + barangs[0].getStok() + ")");
        System.out.println(barangs[1].getNamaBarang() + " (" + barangs[1].getStok() + ")");
    }

    void pengadaan() {
        initBarang();

        // Tambah stok menggunakan method tambahStok()
        barangs[0].tambahStok(20);

        // Tidak bisa lagi mengurangi, mengali, atau membagi stok secara langsung
        // barangs[0].stok -= 30;  --> ini tidak bisa dilakukan lagi
        // barangs[0].stok *= 30;  --> ini juga tidak bisa dilakukan lagi

        showBarang();
    }

    public static void main(String[] args) {
        Inventory beli = new Inventory();
        beli.pengadaan();
    }
}
